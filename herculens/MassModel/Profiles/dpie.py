...
class DPIE_GLEE(object):
    ...
    def __init__(self, r_soft=1e-8, scale_flag=True):
        self._r_soft = r_soft
        self._flag_dpie = scale_flag # if True, theta_E corresponds to the Einstein radius of the profile
        self._flag_piemd = False  # this needs to be always False to recover the GLEE behavior
        self._init_piemd = lambda x, y: Piemd_GPU(0., 0., 0., 0., xx=x, yy=y)
    ...
    def function(self, x, y, theta_E, r_core, r_trunc, q, phi, center_x=0, center_y=0):
        ...
        """
        piemd = self._init_piemd(x, y)
        theta_E_piemd, w, s = self._param_conv(theta_E, r_core, r_trunc, self._flag_dpie)
        f_w = piemd._potential(center_x, center_y, q, phi, theta_E_piemd, w, self._flag_piemd)
        f_s = piemd._potential(center_x, center_y, q, phi, theta_E_piemd, s, self._flag_piemd)
        f = f_w - f_s
    ...
    def derivatives(self, x, y, theta_E, r_core, r_trunc, q, phi, center_x=0, center_y=0):
        ...
        piemd = self._init_piemd(x, y)
        theta_E_piemd, w, s = self._param_conv(theta_E, r_core, r_trunc, self._flag_dpie)
        f_x_w, f_y_w = piemd._deflection_angle(center_x, center_y, q, phi, theta_E_piemd, w, self._flag_piemd)
        f_x_s, f_y_s = piemd._deflection_angle(center_x, center_y, q, phi, theta_E_piemd, s, self._flag_piemd)
        f_x = f_x_w - f_x_s
        f_y = f_y_w - f_y_s
    ...
    def hessian(self, x, y, theta_E, r_core, r_trunc, q, phi, center_x=0, center_y=0):
        ...
        piemd = self._init_piemd(x, y)
        theta_E_piemd, w, s = self._param_conv(theta_E, r_core, r_trunc, self._flag_dpie)
        f_xx_w, f_yy_w, f_xy_w = piemd._hessian(center_x, center_y, q, phi, theta_E_piemd, w, self._flag_piemd)
        f_xx_s, f_yy_s, f_xy_s = piemd._hessian(center_x, center_y, q, phi, theta_E_piemd, s, self._flag_piemd)
        f_xx = f_xx_w - f_xx_s
        f_yy = f_yy_w - f_yy_s
        f_xy = f_xy_w - f_xy_s
...
#         :return: alpha_x, alpha_y
#         """
#         theta_E_scl, w, s = self._param_conv(theta_E, r_core, r_trunc, self._flag_dpie)
