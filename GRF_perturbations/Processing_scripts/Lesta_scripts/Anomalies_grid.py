...
def get_anomalies_grid(logA_to_process,Beta_to_process,Inference):

    compute_anomalies_pure=lambda logA,Beta,Fourier_phase: Compute_anomalies(Inference,jnp.array([logA,Beta]),Fourier_phase)
    ...
    compute_anomalies_Betas= jax.jit(lambda logA: jax_map(lambda Beta: compute_anomalies_seeds(logA,Beta),Beta_to_process))
...
if __name__ == '__main__':
    ...
    for i in range(0,len(Beta_array),16):

        Beta_to_process=Beta_array[i:i+16]
	
        for b in Beta_to_process:
            os.system('echo j={:.0f} Beta={:.2f}'.format(i,b))
        anomalies_grid=get_anomalies_grid(logA_to_process,Beta_to_process,Inference)
        np.savez('./results/anomalies_grid_node_%d_batch_%d'%(node_num,i),anomalies_grid)
