...
class PIEMD(object):
    def __init__(self, r_soft=1e-8, scale_flag=True):
        ...
        In the Herculens code below, we use theta_E parameter for E0 (or Elim), phi for the position angle, and q for the axis ratio, 
    ...
    def function(self, x, y, theta_E, r_core, q, phi, center_x=0, center_y=0):  # flags should be also an array
        ...
        return jax.lax.cond(e != 0, self._poten_q, self._poten_spherical, x, y, center_x, center_y, e, phi, theta_E, w)
    ...
    def derivatives(self, x, y, theta_E, r_core, q, phi, center_x=0, center_y=0):
        ...
        w = r_core
        theta_E = jnp.where(self._scale_flag, theta_E**2/(jnp.sqrt(theta_E*theta_E + w * w) - w), theta_E)
        return jax.lax.cond(e!=0, self._def_q, self._def_spherical, x, y, center_x, center_y, e, phi, theta_E, w)
    ...
    def hessian(self, x, y, theta_E, r_core, q, phi, center_x=0, center_y=0):
        ...
        w = r_core
        theta_E = jnp.where(self._scale_flag, theta_E**2/(jnp.sqrt(theta_E*theta_E + w * w) - w), theta_E)
        return jax.lax.cond(e!=0, self._hessian_q, self._hessian_spherical, x, y, center_x, center_y, e, phi, theta_E, w)
    
    @partial(jax.jit, static_argnums=(0,))
    def kappa(self, x, y, theta_E, r_core, q, phi, center_x=0, center_y=0):
        xx = x - center_x
        yy = y - center_y
        xx, yy = self._rotation(xx, yy, phi)
        r2 = xx**2 + yy**2/(q*q)
        w = r_core
        theta_E = jnp.where(self._scale_flag, theta_E**2/(jnp.sqrt(theta_E*theta_E + w * w) - w), theta_E)
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _poten_q(self, x, y, x_centre, y_centre, e, pa, theta_E, w):
        # potential, given q != 1
        xx = x - x_centre
        yy = y - y_centre
        xx, yy = self._rotation(xx, yy, pa)
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _poten_spherical(self, x, y, x_centre, y_centre, _, pa, theta_E, w):
        # potential, given q = 1
        xx = x - x_centre
        yy = y - y_centre
        xx, yy = self._rotation(xx, yy, pa)
    ...
    def _def_q(self, x, y, x_centre, y_centre, e, pa, E0, w):
        # return deflection_angle, given q!=1
        dx =  x - x_centre
        dy =  y - y_centre
        dx, dy = self._rotation(dx, dy, pa)
        ...
        alp_x, alp_y = self._rotation(alp_x, alp_y, -pa)
        return alp_x, alp_y

    def _def_spherical(self, x, y, x_centre, y_centre, e, pa, theta_E, w):
    ...
    def _hessian_q(self, x, y, x_centre, y_centre, e, pa ,theta_E, w):
        xx =  x - x_centre
        yy =  y - y_centre
        dx, dy = self._rotation(xx, yy, pa)
